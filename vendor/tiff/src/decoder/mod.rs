...
impl<R: Read + Seek> Decoder<R> {
    ...
    /// Loads the image directory at an explicit IFD pointer.
    ///
    /// This is useful for TIFF features such as `SubIFD`, where reduced-resolution
    /// images are referenced by raw directory offsets rather than participating in
    /// the decoder's main image chain.
    pub fn seek_to_ifd_pointer(&mut self, ifd_pointer: IfdPointer) -> TiffResult<()> {
        let ifd = self.value_reader.read_directory(ifd_pointer)?;
        self.current_ifd = Some(ifd_pointer);
