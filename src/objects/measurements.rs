...
impl ObjectsLayer {
    ...
    pub fn control_measurement_state_json(&self, dataset: &OmeZarrDataset) -> serde_json::Value {
        let target_count = self.bulk_measurement_target_indices().len();
        serde_json::json!({
            "running": self.is_bulk_measuring(),
            "status": self.bulk_measurement_status,
            "metric": bulk_measurement_metric_label(self.bulk_measurement_metric),
            "level": self.bulk_measurement_level,
            "concurrency": self.bulk_measurement_concurrency,
            "filtered_only": self.bulk_measurement_filtered_only,
            ...
            "progress": {
                "completed": self.bulk_measurement_progress_completed,
                "total": self.bulk_measurement_progress_total,
            },
            "levels": dataset.levels.iter().map(|level| {
                let width = level.shape.get(dataset.dims.x).copied();
                let height = level.shape.get(dataset.dims.y).copied();
                serde_json::json!({
                    "index": level.index,
                    "downsample": level.downsample,
                    "width": width,
                    "height": height,
                    "label_raster_bytes": width.zip(height).map(|(width, height)| width.saturating_mul(height).saturating_mul(4)),
                })
            }).collect::<Vec<_>>(),
            "generated_properties": self.scalar_property_keys.iter()
                .filter(|key| key.starts_with(&self.bulk_measurement_prefix))
                .cloned()
                .collect::<Vec<_>>(),
        })
    }

    pub fn control_configure_measurement_json(
        &mut self,
        params: &serde_json::Value,
        dataset: &OmeZarrDataset,
    ) -> serde_json::Value {
        if self.is_bulk_measuring() {
            return serde_json::json!({"error": "cannot reconfigure measurements while a run is active"});
        }
        let metric = match params.get("metric").and_then(serde_json::Value::as_str) {
            Some("mean") => Some(BulkMeasurementMetric::Mean),
            Some("median" | "exact_median") => Some(BulkMeasurementMetric::Median),
            Some(_) => return serde_json::json!({"error": "metric must be 'mean' or 'median'"}),
            None => None,
        };
        let level = match params.get("level") {
            Some(value) => {
                let Some(level) = value
                    .as_u64()
                    .and_then(|value| usize::try_from(value).ok())
                    .filter(|level| *level < dataset.levels.len())
                else {
                    return serde_json::json!({"error": "measurement level is out of range"});
                };
                Some(level)
            }
            None => None,
        };
        let concurrency = match params.get("concurrency") {
            Some(value) => {
                let Some(value) = value
                    ...
                    return serde_json::json!({"error": "concurrency must be an integer from 1 to 64"});
                };
                Some(value)
            }
            None => None,
        ...
        let prefix = match params.get("prefix") {
            Some(value) => {
                let Some(value) = value
                    ...
                    return serde_json::json!({"error": "prefix must be a non-empty string"});
                };
        ...
        if let Some(metric) = metric {
            self.bulk_measurement_metric = metric;
            if prefix.is_none() {
                self.bulk_measurement_prefix = default_bulk_measurement_prefix(metric).to_string();
            }
        }
        ...
        if let Some(filtered_only) = params
            .get("filtered_only")
            .and_then(serde_json::Value::as_bool)
        {
            self.bulk_measurement_filtered_only = filtered_only;
        ...
        if let Some(prefix) = prefix {
            self.bulk_measurement_prefix = prefix;
        }
        self.control_measurement_state_json(dataset)
    }

    pub fn control_start_measurement_json(
        &mut self,
        params: &serde_json::Value,
        dataset: &OmeZarrDataset,
        store: Arc<dyn ReadableStorageTraits>,
        ...
        let configured = self.control_configure_measurement_json(params, dataset);
        if configured.get("error").is_some() {
            return configured;
        }
        self.request_bulk_measurement(dataset, store, channels, local_to_world_offset);
        if !self.is_bulk_measuring() {
            return serde_json::json!({"error": self.bulk_measurement_status});
        }
        serde_json::json!({
            "started": true,
            "measurement": self.control_measurement_state_json(dataset),
        })
    }

    pub fn control_cancel_measurement_json(&mut self) -> serde_json::Value {
        let cancelled = if let Some(cancel) = self.bulk_measurement_cancel.as_ref() {
            cancel.store(true, Ordering::Relaxed);
            self.bulk_measurement_status = "Cancelling measurements...".to_string();
            true
        } else {
        ...
        serde_json::json!({"cancelled": cancelled, "status": self.bulk_measurement_status})
    }
}
