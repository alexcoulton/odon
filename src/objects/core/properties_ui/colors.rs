...
impl ObjectsLayer {
    ...
    pub(in crate::objects::core) fn apply_loaded_property_values(
        ...
        self.apply_loaded_property_column(property_key, column);
    }

    pub(in crate::objects::core) fn apply_loaded_property_payload(
        &mut self,
        property_key: &str,
        property_values: &LoadedPropertyValues,
    ) {
        let Some(objects) = self.objects.as_ref() else {
            return;
        };
        let column = object_property_column_from_loaded_values(objects, property_values);
        self.apply_loaded_property_column(property_key, column);
    }

    fn apply_loaded_property_column(&mut self, property_key: &str, column: ObjectPropertyColumn) {
