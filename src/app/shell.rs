//! Native realization of the actor-owned single-view shell projection.
...
impl OmeZarrViewerApp {
    ...
    pub(crate) fn apply_control_shell_projection(&mut self, shell: &serde_json::Value) {
        ...
        for plan in crate::ui::shell_layout::native_panel_plans(shell, "builtin:viewer-canvas") {
            match plan.selected_mount.as_deref() {
                ...
                Some("builtin:analysis") => {
                    self.right_tab = RightTab::Analysis;
                }
                Some("builtin:measurements") => {
                    self.right_tab = RightTab::Measurements;
                }
                Some("builtin:memory") => {
                    self.right_tab = RightTab::Memory;
                }
                Some("builtin:roi-selector") => {
                    self.right_tab = RightTab::RoiSelector;
                }
                _ => {}
            }
        }
        ...
        let mut order = if has_layout {
            layout_order
                ...
                .filter_map(|mount| match mount.as_str() {
                    "builtin:layers" => Some("builtin:single.layers".to_string()),
                    "builtin:project" => Some("builtin:single.project".to_string()),
                    _ => None,
                })
                .collect()
        } else {
            ordered_children(&self.control_shell_projection, "builtin:single.left-tabs")
        };
        if order.is_empty() && !has_layout {
            order = vec![
                "builtin:single.layers".into(),
                "builtin:single.project".into(),
            ];
        }
        order
            .iter()
            .filter_map(|id| match id.as_str() {
                "builtin:single.layers" => Some(left_panel::TabSpec {
                    tab: LeftTab::Layers,
                    label: "Layers",
                    panel_key: "layers",
                ...
                "builtin:single.project" => Some(left_panel::TabSpec {
                    tab: LeftTab::Project,
                    label: "Project",
                    panel_key: "project",
    ...
    pub(super) fn shell_right_tabs(&self) -> Vec<right_panel::TabSpec<RightTab>> {
        ...
        let mut order = if has_layout {
            layout_order
                .into_iter()
                .filter_map(|mount| match mount.as_str() {
                    "builtin:properties" => Some("builtin:single.properties".to_string()),
                    "builtin:views" => Some("builtin:single.views".to_string()),
                    "builtin:analysis" => Some("builtin:single.analysis".to_string()),
                    "builtin:measurements" => Some("builtin:single.measurements".to_string()),
                    "builtin:memory" => Some("builtin:single.memory".to_string()),
                    "builtin:roi-selector" => Some("builtin:single.roi-selector".to_string()),
                    _ => None,
                })
                .collect()
        } else {
            ordered_children(&self.control_shell_projection, "builtin:single.right-tabs")
        };
        if order.is_empty() && !has_layout {
            order = [
                "properties",
            ...
            .into_iter()
            .map(|id| format!("builtin:single.{id}"))
            .collect();
        }
        order
            .iter()
            .filter_map(|id| {
                let (tab, label) = match id.as_str() {
                    "builtin:single.properties" => (RightTab::Properties, "Properties"),
                    "builtin:single.views" => (RightTab::Views, "Views"),
                    "builtin:single.analysis" => (RightTab::Analysis, "Analysis"),
                    "builtin:single.measurements" => (RightTab::Measurements, "Measurements"),
                    "builtin:single.memory" => (RightTab::Memory, "Memory"),
                    "builtin:single.roi-selector" => (RightTab::RoiSelector, "ROI Selector"),
                    _ => return None,
                };
    ...
    pub(super) fn ui_actor_shell_tree(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) -> bool {
        ...
        let changes = frame.show(ui, |mount_ui, mount, configuration| match mount {
            "builtin:viewer-top-bar" => {
                self.ui_viewer_top_bar(mount_ui, ctx, configuration)
            }
            "builtin:viewer-viewport-controls" => {
                mount_ui.horizontal(|ui| self.ui_viewport_controls(ui));
            }
            ...
            "builtin:viewer-canvas" => self.ui_viewport_workspace(mount_ui, ctx),
            "builtin:shell-inspector" => crate::ui::shell_inspector::render(
                mount_ui,
                &projection,
                &shell_geometry,
            ),
            "builtin:help" => crate::ui::help::render_help_browser(mount_ui, "single-shell"),
...
fn legacy_mount(id: &str) -> Option<&'static str> {
    match id {
        "builtin:single.top-bar" => Some("builtin:viewer-top-bar"),
        "builtin:single.canvas" => Some("builtin:viewer-canvas"),
        "builtin:single.layers" => Some("builtin:layers"),
        "builtin:single.project" => Some("builtin:project"),
        "builtin:single.properties" => Some("builtin:properties"),
        "builtin:single.views" => Some("builtin:views"),
        "builtin:single.analysis" => Some("builtin:analysis"),
        "builtin:single.measurements" => Some("builtin:measurements"),
        "builtin:single.memory" => Some("builtin:memory"),
        "builtin:single.roi-selector" => Some("builtin:roi-selector"),
        _ => None,
    }
}
...
fn layout_panel_plan(
    ...
    crate::ui::shell_layout::native_panel_plans(shell, "builtin:viewer-canvas")
        .into_iter()
