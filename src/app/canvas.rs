...
impl OmeZarrViewerApp {
    pub(super) fn ui_canvas(
        ...
        // `allocate_ui_with_layout` constrains layout but can inherit the parent central-panel
        // paint clip. In a split workspace that lets CPU shapes and GL callbacks from one
        // viewport survive into its sibling. Make the allocated canvas the hard clip boundary
        // before any image, mask, annotation, selection, HUD, or callback is painted.
        ...
        let camera_changed =
            self.handle_canvas_interactions(ui, ctx, rect, &response, allows_edits);
