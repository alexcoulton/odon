...
fn motivating_two_property_comparison_runs_end_to_end_on_one_document() {
    ...
    let resource = crate::objects::load_control_object_resource(object_path.clone(), 1.0).unwrap();
    let mut app = ActorAppFixture::new(app);
    app.install_object_resource(resource);
    ...
    assert_eq!(left_style["result"]["style"]["color_property"], "marker_a");
    assert_eq!(right_style["result"]["style"]["color_property"], "marker_b");
    let left_value = "1";
    let right_value = "9";
    app.actor_command(
    ...
    app.actor_command(
        ...
        serde_json::json!({"viewport_id": right, "mode": "xy", "slice": 0}),
    );
    let selected = app.actor_command(
    ...
    assert_eq!(
        app.actor_query(
            ...
            serde_json::json!({"target": "objects"})
